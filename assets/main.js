...
const updateWeatherData = async (city) => {
    ...
    let currentWeatherData = await getCurrentWeatherData(locationData[0].lat, locationData[0].lon);

    // Render the weather data
    renderWeatherItems(locationData[0].name, weatherData, currentWeatherData);
...
const getCurrentWeatherData = async (lat, lon) => {
    const response = await fetch(`${currentWeatherApi}?lat=${lat}&lon=${lon}&units=imperial&appid=${apiKey}`);
    const data = await response.json();
    return data;
}
...
const renderWeatherItems = (city, data, currentWeatherData) => {
    const currentDate = new Date(data.list[0].dt_txt);
    const currentWeatherIcon = currentWeatherData.weather[0].icon;
    ...
    const mainCard = `<div class="card weather-main-card">
                        <div class="card-body">
                            ...
                            <p class="city-temp my-1 fw-semibold">
                                Temperature: <span>${currentWeatherData.main.temp}&deg;F / ${farenheitToCelcius(currentWeatherData.main.temp)}&deg;C</span>
                            </p>

                            <p class="city-humidity my-1 fw-semibold">
                                Humidity: <span>${currentWeatherData.main.humidity}%</span>
                            </p>

                            <p class="city-wind my-1 fw-semibold">
                                Wind: <span>${currentWeatherData.wind.speed} MPH / ${mphToKph(currentWeatherData.wind.speed)} KPH</span>
                            </p>
                        </div>
                    </div>`;
